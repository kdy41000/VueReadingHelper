...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/admin/**").hasRole("ADMIN") // Admin 권한이 있는 경우 접근 허용
				...
				.antMatchers("/", "/loginRes", "/login/loginRes", "/admin/**", "/signup", "/loginprocess","/signupprocess","/resources/**").permitAll() // 해당 URL은 전체 접근 허용
				...
				.defaultSuccessUrl("http://localhost:8081/#/main")    //로그인 성공시 해당 어노테이션 호출
				...
				.logoutSuccessUrl("/")   //로그아웃 성공시 해당 어노테이션 호출
